...
# --- 🗓️ [업그레이드] 달력 날짜 제어 로직 ---
# 1. 데이터에 있는 날짜들(문자열)을 가져와서 실제 날짜 객체로 변환
...
# 3. 전체 기간 중 데이터가 없는 날(주말/휴장일) 리스트 만들기
...
# 4. 오늘 날짜가 데이터에 있으면 오늘을, 없으면 가장 가까운 미래/과거 데이터 날짜를 기본값으로
today = date.today()
...
with tab1:
    ...
    with col_d:
        # [수정] disabled_dates를 추가하여 주말/휴장일 선택 막기
        sel_date_obj = st.date_input("날짜 선택", 
                                     value=default_date, 
                                     min_value=min_date, 
                                     max_value=max_date, 
...
with tab2:
    ...
    # [수정] 시작일 선택 시에도 휴장일 방지
    st_date_obj = st.date_input("시작일", 
                                 value=default_date, 
                                 min_value=min_date, 
                                 max_value=max_date, 
                                 disabled_dates=disabled_days, 
                                 key="t2_date")
    st_date = st_date_obj.strftime('%Y-%m-%d')
...
with tab3:
    ...
    with d3_col:
        # [수정] 전체 현황 시작일 선택 시에도 휴장일 방지
        st_date3_obj = st.date_input("시작일", 
                                      value=default_date, 
                                      min_value=min_date, 
                                      max_value=max_date, 
                                      disabled_dates=disabled_days, 
                                      key="t3_date")
        st_date3 = st_date3_obj.strftime('%Y-%m-%d')
    with c3_col:
